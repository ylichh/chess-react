...
type RangoPosibleStrategy = (
  argumentosMovimientoEnRango: MovimientoEnRangoArgumentos
) => CasillaInterface[];

export function movimientoEnRango(
  argumentosMovimientoEnRango: MovimientoEnRangoArgumentos
): boolean {
  let direccionSentido = obtenDireccionSentido(
    argumentosMovimientoEnRango.casillaOrigen,
    argumentosMovimientoEnRango.casillaDestino
  );
  ...
  return rango.some(
    (rango) =>
      rango.getFila() ===
        argumentosMovimientoEnRango.casillaDestino.getFila() &&
      rango.getColumna() ===
        argumentosMovimientoEnRango.casillaDestino.getColumna()
  );
}

const rangoPosiblePeon: RangoPosibleStrategy = (
  argumentosMovimientoEnRango
) => {
  ...
  rangoPosible = rangoPosible.concat(
    _rangoMovimiento(
      argumentosMovimientoEnRango.casillaOrigen,
      argumentosMovimientoEnRango.posicionTablero
    )
  );
  rangoPosible = rangoPosible.concat(
    _rangoCapturaClasica(
      argumentosMovimientoEnRango.casillaOrigen,
      argumentosMovimientoEnRango.posicionTablero
    )
  );
  rangoPosible = rangoPosible.concat(
    _rangoCapturaAlPaso(
      argumentosMovimientoEnRango.casillaOrigen,
      argumentosMovimientoEnRango.movimientosEspeciales.alPaso
    )
  );
  return rangoPosible;
};

const _rangoMovimiento = (
  casillaOrigen: CasillaInterface,
  posicionTablero: TableroInterface
) => {
...
const _rangoCapturaAlPaso = (
  casillaOrigen: CasillaInterface,
  casillaAlPasoDisponible: CasillaInterface[]
) => {
  if (
    casillaAlPasoDisponible.length == 0 ||
    casillaOrigen.getPieza() !== casillaAlPasoDisponible[0].getPieza()
  ) {
    return [];
  }
  ...
  let direccionOrigen =
    casillaAlPaso?.getPieza() === PIEZAS.PEON_BLANCO ? -1 : +1;
...
const rangoPosibleCaballo: RangoPosibleStrategy = (
  argumentosMovimientoEnRango: MovimientoEnRangoArgumentos
) => {
  let rangoPosible: Casilla[] = [];

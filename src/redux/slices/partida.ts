...
export const estadoPartidaSlice = createSlice({
  ...
  reducers: {
    ...
    actualizarPosicionTablero: (state, action) => {
      // state.posicionTablero.actualizaPosicion(action.payload.piezaTocada, action.payload.casillaDestino);
      state.posicionTablero = state.posicionTablero.actualizaPosicion(action.payload.piezaTocada, action.payload.casillaDestino);
      state.turno++;
    },
